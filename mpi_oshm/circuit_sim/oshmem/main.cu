...
/* oshmem init */
    int rank, commSize;
    start_pes(0);     //MPI_Init(&argc, &argv);
    rank = _my_pe();  //MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    commSize = _num_pes(); //MPI_Comm_size(MPI_COMM_WORLD, &commSize); 
    if (rank == 0) {
        printf("===========\nnum_loops:%d\nnum_pieces:%d\nnodes_per_piece:%d\nwires_per_piece:%d\n==========\n", num_loops, num_pieces, nodes_per_piece, wires_per_piece);
    }
    printf("My Rank: %d, total pe: %d\n", rank, commSize);
    num_pe = commSize;
...
/* circuit graph building */
    ...
    node_piece  = (node*) shmalloc(sizeof(node)*num_pieces);//new node[num_pieces];
    wire_piece = (wire*) shmalloc(sizeof(wire)*num_pieces);//new wire[num_pieces];
    for (int n = 0; n < num_pieces; n++) {
        // allocate space for array in soa of node
        node_piece[n].capacitance = (PRECISION *) shmalloc(sizeof(PRECISION)*nodes_per_piece);//new PRECISION[nodes_per_piece];
        node_piece[n].leakage     = (PRECISION *) shmalloc(sizeof(PRECISION)*nodes_per_piece);//new PRECISION[nodes_per_piece];
        node_piece[n].charge      = (PRECISION *) shmalloc(sizeof(PRECISION)*nodes_per_piece);//new PRECISION[nodes_per_piece];
        node_piece[n].voltage     = (PRECISION *) shmalloc(sizeof(PRECISION)*nodes_per_piece);//new PRECISION[nodes_per_piece];
        node_piece[n].shr_pc      = (int *) shmalloc(sizeof(int)*nodes_per_piece);//new int[nodes_per_piece];
        node_piece[n].node_attr   = (int *) shmalloc(sizeof(int)*nodes_per_piece);//new int[nodes_per_piece];
        // allocate space for array in soa of wire
        wire_piece[n].currents    = (PRECISION **) shmalloc(sizeof(PRECISION*)*wires_per_piece);//new PRECISION*[wires_per_piece];
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].currents[j]  = (PRECISION *) shmalloc(sizeof(PRECISION)*WIRE_SEGMENTS);//new PRECISION[WIRE_SEGMENTS];
        wire_piece[n].voltages    = (PRECISION **) shmalloc(sizeof(PRECISION*)*wires_per_piece);//new PRECISION*[wires_per_piece];
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].voltages[j]  = (PRECISION *) shmalloc(sizeof(PRECISION)*(WIRE_SEGMENTS-1));//new PRECISION[WIRE_SEGMENTS-1];
        wire_piece[n].resistance  = (PRECISION *) shmalloc(sizeof(PRECISION)*wires_per_piece);//new PRECISION[wires_per_piece];
        wire_piece[n].inductance  = (PRECISION *) shmalloc(sizeof(PRECISION)*wires_per_piece);//new PRECISION[wires_per_piece];
        wire_piece[n].capacitance = (PRECISION *) shmalloc(sizeof(PRECISION)*wires_per_piece);//new PRECISION[wires_per_piece];
        wire_piece[n].in_ptr      = (int *) shmalloc(sizeof(int)*wires_per_piece);//new int[wires_per_piece];
        wire_piece[n].out_ptr     = (int *) shmalloc(sizeof(int)*wires_per_piece);//new int[wires_per_piece];
        wire_piece[n].wire_attr   = (int *) shmalloc(sizeof(int)*wires_per_piece);//new int[wires_per_piece];
        // init wire shared part
        wire_piece[n].shr_voltage = (PRECISION *) shmalloc(sizeof(PRECISION)*wires_per_piece);//new PRECISION[wires_per_piece];
        wire_piece[n].shr_charge  = (PRECISION *) shmalloc(sizeof(PRECISION)*wires_per_piece);//new PRECISION[wires_per_piece];
        wire_piece[n].shr_pc      = (int *) shmalloc(sizeof(int)*wires_per_piece);//new int[wires_per_piece];
    }
    if (rank == 0) {
...
//                wire_piece[n].back().out_ptr = random_element(nodes_per_piece);
    ...
    if (rank) {
        for (int n=0; n<pieces_per_pe; n++) {
            int noffset = (rank-1)*pieces_per_pe + n;
            // send node info
            shmem_float_get(node_piece[n].capacitance,node_piece[noffset].capacitance, nodes_per_piece, 0);
            shmem_float_get(node_piece[n].leakage   ,node_piece[noffset].leakage    , nodes_per_piece, 0);
            shmem_float_get(node_piece[n].charge    ,node_piece[noffset].charge     , nodes_per_piece, 0);
            shmem_float_get(node_piece[n].voltage   ,node_piece[noffset].voltage    , nodes_per_piece, 0);
            shmem_int_get(node_piece[n].shr_pc    ,node_piece[noffset].shr_pc     , nodes_per_piece, 0);
            shmem_int_get(node_piece[n].node_attr ,node_piece[noffset].node_attr  , nodes_per_piece, 0);
            // send wire info
            for (int i=0; i<wires_per_piece; i++)
                shmem_float_get(wire_piece[n].currents[i], wire_piece[noffset].currents[i], WIRE_SEGMENTS, 0);
            for (int i=0; i<wires_per_piece; i++)
                shmem_float_get(wire_piece[n].voltages[i], wire_piece[noffset].voltages[i], WIRE_SEGMENTS-1, 0);
            shmem_float_get(wire_piece[n].resistance , wire_piece[noffset].resistance , wires_per_piece, 0);
            shmem_float_get(wire_piece[n].inductance , wire_piece[noffset].inductance , wires_per_piece, 0);
            shmem_float_get(wire_piece[n].capacitance, wire_piece[noffset].capacitance, wires_per_piece, 0);
            shmem_int_get(wire_piece[n].in_ptr     , wire_piece[noffset].in_ptr     , wires_per_piece, 0);
            shmem_int_get(wire_piece[n].out_ptr    , wire_piece[noffset].out_ptr    , wires_per_piece, 0);
            shmem_int_get(wire_piece[n].wire_attr  , wire_piece[noffset].wire_attr  , wires_per_piece, 0);
            shmem_float_get(wire_piece[n].shr_voltage, wire_piece[noffset].shr_voltage, wires_per_piece, 0);
            shmem_float_get(wire_piece[n].shr_charge , wire_piece[noffset].shr_charge , wires_per_piece, 0);
            shmem_int_get(wire_piece[n].shr_pc    , wire_piece[noffset].shr_pc      , wires_per_piece, 0);
        }  
...
*/
    ...
    /* post work for charge distribution */
            ...
            for (int i=0; i<pieces_per_pe; i++) {
                int noffset = (rank-1)*pieces_per_pe + i;
#ifdef SHM
                shmem_float_put(node_piece[noffset].charge    , node_piece[i].charge    , nodes_per_piece, 0);
                shmem_float_put(wire_piece[noffset].shr_charge, wire_piece[i].shr_charge, wires_per_piece, 0);
#endif
            }
        } 
        shmem_barrier_all();//MPI_Barrier(MPI_COMM_WORLD);
        ...
        if (rank) { 
            ...
            for (int i=0; i<pieces_per_pe; i++) {
                int noffset = (rank-1)*pieces_per_pe + i;
#ifdef SHM
                shmem_float_get(node_piece[i].charge    , node_piece[noffset].charge    , nodes_per_piece, 0);
#endif
            }
            for (int i=0; i<pieces_per_pe; i++) {
                for (int j=0; j<wires_per_piece; j++) {
    ...
    /* post_work for updating voltage */
            ...
            for (int i=0; i<pieces_per_pe; i++) {
                int noffset = (rank-1)*pieces_per_pe + i;
#ifdef SHM
                shmem_float_put(node_piece[noffset].voltage  , node_piece[i].voltage   , nodes_per_piece, 0);
#endif
            }
