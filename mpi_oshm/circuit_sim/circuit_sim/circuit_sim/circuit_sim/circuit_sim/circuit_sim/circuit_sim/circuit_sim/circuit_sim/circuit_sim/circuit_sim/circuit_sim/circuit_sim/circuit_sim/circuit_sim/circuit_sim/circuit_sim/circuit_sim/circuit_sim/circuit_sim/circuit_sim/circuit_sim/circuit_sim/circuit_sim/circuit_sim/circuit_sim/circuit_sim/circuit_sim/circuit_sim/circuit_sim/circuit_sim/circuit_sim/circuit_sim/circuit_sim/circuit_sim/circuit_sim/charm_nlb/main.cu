...
/* circuit graph building */
    ...
    int mem_pc_size = nodes_per_piece * (sizeof(PRECISION) * 4 + sizeof(int) * 2) 
                 + wires_per_piece * (sizeof(PRECISION) * (2 * WIRE_SEGMENTS + 4) + sizeof(int) * 4);
    int mem_size = num_pieces * mem_pc_size;
    mem_pool  = (unsigned char*)malloc(mem_size);
    mem_begin = mem_pool; 
    // Open MPI win for shared variables
    MPI_Win win;
    MPI_Win_create(mem_pool, mem_size, sizeof(unsigned char), MPI_INFO_NULL, MPI_COMM_WORLD, &win);    
    ...
    for (int n = 0; n < num_pieces; n++) {
        ...
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].currents[j]  = reinterpret_cast<PRECISION *>(&mem_pool[j*WIRE_SEGMENTS]);
        mem_pool += sizeof(PRECISION) * wires_per_piece * WIRE_SEGMENTS;
        wire_piece[n].voltages    = (PRECISION **)new PRECISION*[wires_per_piece];
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].voltages[j]  = reinterpret_cast<PRECISION *>(&mem_pool[j*(WIRE_SEGMENTS-1)]);
...
#if 0
    unsigned char * mem_pool, * mem_begin;
    int mem_size = nodes_per_piece * (sizeof(PRECISION) * 4 + sizeof(int) * 2) 
                 + wires_per_piece * (sizeof(PRECISION) * (2 * WIRE_SEGMENTS + 4) + sizeof(int) * 4);
    //MPI_Alloc_mem(mem_size, MPI_INFO_NULL, &mem_pool); 
    mem_pool  = (unsigned char*)malloc(mem_size);
    mem_begin = mem_pool; 
    for (int n = 0; n < num_pieces; n++) {
        // allocate space for array in soa
        node_piece[n].capacitance = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * nodes_per_piece;
        node_piece[n].leakage     = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * nodes_per_piece;
        node_piece[n].charge      = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * nodes_per_piece;
        node_piece[n].voltage     = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * nodes_per_piece;
        node_piece[n].shr_pc      = (int *) mem_pool;
        mem_pool += sizeof(int) * nodes_per_piece;
        node_piece[n].node_attr   = (int *) mem_pool;
        mem_pool += sizeof(int) * nodes_per_piece;
        // allocate space for array in soa of wire
        wire_piece[n].currents    = (PRECISION **)new PRECISION[wires_per_piece];
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].currents[j]  = (PRECISION *)&mem_pool[j*WIRE_SEGMENTS];
        mem_pool += sizeof(PRECISION) * wires_per_piece * WIRE_SEGMENTS;
        wire_piece[n].voltages    = (PRECISION **)new PRECISION[wires_per_piece];
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].voltages[j]  = (PRECISION *)&mem_pool[j*(WIRE_SEGMENTS-1)];
        mem_pool += sizeof(PRECISION) * wires_per_piece * (WIRE_SEGMENTS-1);
        wire_piece[n].resistance  = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * wires_per_piece;
        wire_piece[n].inductance  = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * wires_per_piece;
        wire_piece[n].capacitance = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * wires_per_piece;
        wire_piece[n].in_ptr      = (int *)mem_pool;
        mem_pool += sizeof(int) * wires_per_piece;
        wire_piece[n].out_ptr     = (int *)mem_pool;
        mem_pool += sizeof(int) * wires_per_piece;
        wire_piece[n].wire_attr   = (int *)mem_pool;
        mem_pool += sizeof(int) * wires_per_piece;
        // init wire shared part
        wire_piece[n].shr_voltage = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * wires_per_piece;
        wire_piece[n].shr_charge  = (PRECISION *)mem_pool;
        mem_pool += sizeof(PRECISION) * wires_per_piece;
        wire_piece[n].shr_pc      = (int *)mem_pool;
    }
#endif
#if 0
    for (int n = 0; n < num_pieces; n++) {
    ...
    PRECISION * mem_attr;
    int       * mem_flag;
    int         mem_attr_size = num_pieces * nodes_per_piece * (sizeof(PRECISION) * 4) 
                 + num_pieces * wires_per_piece * (sizeof(PRECISION) * (2 * WIRE_SEGMENTS + 4));
    int         mem_flag_size  = num_pieces * nodes_per_piece * sizeof(int) * 2 + num_pieces * wires_per_piece * sizeof(int) * 4;
    mem_attr = (PRECISION *) malloc(mem_attr_size);
    mem_flag = (int *) malloc(mem_flag_size);
    int mem_count = 0;
    int flag_count = 0;
    for (int n = 0; n < num_pieces; n++) {
        // allocate space for array in soa
        node_piece[n].capacitance = &mem_attr[mem_count];
        mem_count += nodes_per_piece;
        node_piece[n].leakage     = &mem_attr[mem_count];
        mem_count += nodes_per_piece;
        node_piece[n].charge      = &mem_attr[mem_count];
        mem_count += nodes_per_piece;
        node_piece[n].voltage     = &mem_attr[mem_count];
        mem_count += nodes_per_piece;
        node_piece[n].shr_pc      = &mem_flag[flag_count];
        flag_count += nodes_per_piece;
        node_piece[n].node_attr   = &mem_flag[flag_count];
        flag_count += nodes_per_piece;
        // allocate space for array in soa of wire
        wire_piece[n].currents    = new PRECISION*[wires_per_piece];
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].currents[j]  = &mem_attr[mem_count + j*WIRE_SEGMENTS];
        mem_count += wires_per_piece*WIRE_SEGMENTS;
        wire_piece[n].voltages    = new PRECISION*[wires_per_piece];
        for (int j=0; j<wires_per_piece; j++)
            wire_piece[n].voltages[j]  = &mem_attr[mem_count + j*(WIRE_SEGMENTS-1)];
        mem_count += wires_per_piece*(WIRE_SEGMENTS-1);
        wire_piece[n].resistance  = &mem_attr[mem_count];
        mem_count += wires_per_piece;
        wire_piece[n].inductance  = &mem_attr[mem_count];
        mem_count += wires_per_piece;
        wire_piece[n].capacitance = &mem_attr[mem_count];
        mem_count += wires_per_piece;
        wire_piece[n].in_ptr      = &mem_flag[flag_count];
        flag_count += wires_per_piece;
        wire_piece[n].out_ptr     = &mem_flag[flag_count];
        flag_count += wires_per_piece;
        wire_piece[n].wire_attr   = &mem_flag[flag_count];
        flag_count += wires_per_piece;
        // init wire shared part
        wire_piece[n].shr_voltage = &mem_attr[mem_count];
        mem_count += wires_per_piece;
        wire_piece[n].shr_charge  = &mem_attr[mem_count];
        mem_count += wires_per_piece;
        wire_piece[n].shr_pc      = &mem_flag[flag_count];
        flag_count += wires_per_piece;
    }
#endif
    // window creation
...
//                wire_piece[n].back().out_ptr = random_element(nodes_per_piece);
    ...
    MPI_Win_fence(0, win);
    if (rank) {
        int disp = (rank-1) * pieces_per_pe * mem_pc_size;
        int mem_transfer = pieces_per_pe * mem_pc_size;
        MPI_Get(mem_begin, mem_transfer, MPI_UNSIGNED_CHAR, 0, disp, mem_transfer, MPI_UNSIGNED_CHAR, win); 
    }
    MPI_Win_fence(0, win);
#endif
