...
//                wire_piece[n].back().out_ptr = random_element(nodes_per_piece);
    ...
    MPI_Barrier(MPI_COMM_WORLD);
#if 0
    // Get all data for working PEs
    MPI_Win_fence(0, win);
    if (rank) {
        int disp = 0;/*(rank-1) * pieces_per_pe * mem_pc_size;*/
        //int mem_transfer = pieces_per_pe * mem_pc_size;
        MPI_Get(mem_begin, mem_size, MPI_UNSIGNED_CHAR, 0, disp, mem_size, MPI_UNSIGNED_CHAR, win);
    }
    MPI_Win_fence(0, win);
#endif
...
for (int iloop = 0; iloop < num_loops; iloop++) {
    ...
    if (rank == 0) {
        ...
        //for (int iloop = 0; iloop < num_loops; iloop++) { 
            ...
            while (count < max_pe) {
#ifdef _DEBUG
                    printf("\t\t\tmain PE[%d], currwid[%d]->>init\n", rev_id, count);
#endif
            ...
            for (int n=0; n<num_pieces; n++) {
                for (int i=0; i<nodes_per_piece; i++) {
                    printf("** node info **\n");
                    printf("capacitance: %f, leakage: %f, charge: %f, voltage: %f \n", node_piece[n].capacitance[i], node_piece[n].leakage[i], node_piece[n].charge[i], node_piece[n].voltage[i]);
                }
...
//DEBUG_R
            ...
            for (int n=0; n<num_pieces; n++) {
                ...
                for (int i=0; i<wires_per_piece; i++) {
                    printf("currents:\n");
                    for (int j=0; j<WIRE_SEGMENTS; j++) {
                        printf("%f ", wire_piece[n].currents[i][j]);
                    }
                    printf("\n");
                    //printf("resistance: %f, inductance: %f, capcitance: %f, shr_voltage: %f, shr_charge \n", wire_piece[n].resistance[i], wire_piece[n].inductance[i], wire_piece[n].capacitance[i], wire_piece[n].shr_voltage[i], wire_piece[n].shr_charge);
                }
                for (int i=0; i<wires_per_piece; i++) {
                    printf("voltages:\n");
                    for (int j=0; j<WIRE_SEGMENTS-1; j++) {
                        printf("%f ", wire_piece[n].voltages[i][j]);
                    }
                    printf("\n");
                    //printf("resistance: %f, inductance: %f, capcitance: %f, shr_voltage: %f, shr_charge \n", wire_piece[n].resistance[i], wire_piece[n].inductance[i], wire_piece[n].capacitance[i], wire_piece[n].shr_voltage[i], wire_piece[n].shr_charge);
                }
        ...
        int  pieces_per_work = pe_per_node * pieces_per_pe;
            ...
            while (!end) {
                ...
                control_buf[0] = 1;
#ifdef _DEBUG
                    printf("\t\tPE[%d], currwid[%d]\n", rank, wid);
#endif
                MPI_Send(control_buf, 4, MPI_INT, 0, 0, MPI_COMM_WORLD);
#ifdef _DEBUG
                    printf("\t\tPE[%d], currwid[%d]->>middle\n", rank, wid);
#endif
                ...
                if (count_buf[1] == 1) {
                    ...
                    int disp = wid * pieces_per_pe * mem_pc_size;
                    int pc_this_pe = ((wid+pe_per_node)>max_pe)?(max_pe-wid):pe_per_node;
                    int mem_transfer = pc_this_pe * pieces_per_pe * mem_pc_size;
...
#endif
#if 0
printf("mPE pe_spn: %d, pieces_per_pe: %d, nodes_per_piece: %d\n", pe_spn, pieces_per_pe, nodes_per_piece);
#endif
                            for (int n = 0; n < pieces_per_pe; n++) {
                                int npiece = p*pieces_per_pe + n;
                                // allocate space for array in soa
...
//DEBUG_R children PE spawning
                    ...
                    //Disconnect children PE
                    MPI_Comm_disconnect(&children_comm);
                    // set count_buf[0] to transfer results back to main PE, control_buf[2] for mainPE receive results
                    count_buf[0]   = 1; 
                    control_buf[1] = wid;
                    control_buf[2] = 1;
                }//endif computation
...
//DEBUG_R
    ...
    printf("Rank:%d/%d, on host: %s, device_id: %d, kernel-12 time: %f\n", rank, parent_rank, hostname, parent_rank % deviceCnt, t_kp);
...
int WorkerRoutine_B(int argc, char ** argv) {
    ...
    for (int n = 0; n < pieces_per_pe; n++) {
        ...
        MPI_Recv(wire_piece[n].shr_voltage, wires_per_piece, MPI_PRECISION, 0, 0, parent_comm, MPI_STATUS_IGNORE);
        MPI_Recv(wire_piece[n].shr_charge , wires_per_piece, MPI_PRECISION, 0, 0, parent_comm, MPI_STATUS_IGNORE);
        MPI_Recv(wire_piece[n].shr_pc     , wires_per_piece, MPI_PRECISION, 0, 0, parent_comm, MPI_STATUS_IGNORE);
        */
    }//endfor pieces_per_pe
    ...
    */
    ...
    printf("Rank:%d/%d, on host: %s, device_id: %d, kernel-3 time: %f\n", rank, parent_rank, hostname, parent_rank % deviceCnt, t_kp);
