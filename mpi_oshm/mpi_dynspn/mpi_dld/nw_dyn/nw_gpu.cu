			...
			int * d_score_matrix, cudaStream_t stream, int stream_num, int kernel_type)
{
	...
	switch(kernel_type) {
		case 0: nw_cuda_diagonal(stream, stream_num, pair_num);
				break;
		case 1: nw_cuda_tile(stream, stream_num, pair_num);
				break;
		default:
				break;
	}
...
void nw_cuda_diagonal( cudaStream_t stream, int stream_num, int pair_num) {
	int i = stream_num;
	needleman_cuda_diagonal<<<config.num_blocks, config.num_threads, 0, stream>>>(	
							...
							d_score_matrix[i], d_pos_matrix[i],	pair_num, config.penalty);
        cudaDeviceSynchronize();
}

void nw_cuda_tile( cudaStream_t stream, int stream_num, int pair_num)
{
    ...
    needleman_cuda_init<<< pair_num, 256, 0, stream>>>(d_score_matrix[i], d_pos_matrix[i], d_dim_matrix[i], config.penalty);
    cudaDeviceSynchronize();
    ...
    for( int j = 1; j <= iteration; ++j) {
        ...
        needleman_cuda_tile_upleft<<<config.num_blocks, config.num_threads, 0, stream>>>(
									d_sequence_set1[i], d_sequence_set2[i], d_pos1[i], d_pos2[i],
                                    d_score_matrix[i], d_pos_matrix[i], d_dim_matrix[i], pair_num, j, config.penalty);
        cudaDeviceSynchronize();
    }
