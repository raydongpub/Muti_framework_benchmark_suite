...
//    CHK_ERR(    cudaSetDevice(0));
    ...
    cout << "[" << rank << "]: cudaSetDevice " <<
        rank % deviceCnt << " and width is: " <<
        gpuConfig.width << " and height is: " <<
        gpuConfig.height << " and vec_size is: " <<
        gpuConfig.vec_size << " and result_size:"<< 
        gpuConfig.result_size << " host is: " <<
        hostname << endl;
#endif
...
cudaError_t ComputeMatrixAttributes(int num_blocks, int num_threads) {
    ...
    cout << "\trank = " << gpuConfig.peidx <<", Launching" <<endl;
    ComputeMatrixAttributes_Kernel <<<num_blocks, num_threads>>> (
    ...
    cout << "\trank = " << gpuConfig.peidx <<", kernel complete" << endl;
    gettimeofday(&time_end,NULL);
    time_period = (time_end.tv_sec + time_end.tv_usec * 1e-6) - (time_begin.tv_sec + time_begin.tv_usec * 1e-6);
    printf("\trank = %d, kernel time: %lf\n", gpuConfig.peidx, time_period);
