...
//                printf("Each gets %d\n",config.num_pairs[k]/NumberOfProcessors);
...
#else
        if (Rank==0) {
           memcpy(sequence_set_rank, sequence_set1, pair_num[0]*seq_len*sizeof(char)/NumberOfProcessors);
           memcpy(sequence_set_rank2, sequence_set2, pair_num[0]*seq_len*sizeof(char)/NumberOfProcessors);
           memcpy(pos1_rank, pos1, pair_num[0]*sizeof(unsigned)/NumberOfProcessors);
           memcpy(pos2_rank, pos2, pair_num[0]*sizeof(unsigned)/NumberOfProcessors);
           memcpy(pos_matrix_rank, pos_matrix, pair_num[0]*sizeof(unsigned)/NumberOfProcessors);
        }
        MPI_Barrier(MPI_COMM_WORLD);
        ...
        for (int k=0; k<config.num_streams;k++){
            ...
            MPI_Bcast(sequence_set_rank[k], pair_num[k]*seq_len/                              NumberOfProcessors,MPI_CHAR, 0, MPI_COMM_WORLD);

            MPI_Bcast(sequence_set_rank2[k], pair_num[k]*seq_len/                              NumberOfProcessors,MPI_CHAR, 0, MPI_COMM_WORLD);

        //Scatter position values here        
            MPI_Bcast(pos1_rank[k], pair_num[k]/NumberOfProcessors, MPI_UNSIGNED,
            0,                   MPI_COMM_WORLD);

            MPI_Bcast(pos2_rank[k], pair_num[k]/NumberOfProcessors, MPI_UNSIGNED,
            0,                   MPI_COMM_WORLD);

            MPI_Bcast(pos_matrix_rank[k], pair_num[k]/NumberOfProcessors,
