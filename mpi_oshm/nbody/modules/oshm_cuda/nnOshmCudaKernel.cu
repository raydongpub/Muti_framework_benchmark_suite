...
cudaError_t InitializeParticleLists() {
    ...
    void * res_v;
    size_t res_size = ((10 * hpDataset->mNumParticles) +
        (10 * gpuConfig.localCnt)) * sizeof(PRECISION);
    CHK_ERR(    cudaMalloc(&res_v, res_size));

    char * res = reinterpret_cast<char *>(res_v);

    size_t offset = 0;
    gParList_d.xPos = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.yPos = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.zPos = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.xVel = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.yVel = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.zVel = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.xAcc = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.yAcc = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.zAcc = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
    gParList_d.mass = reinterpret_cast<PRECISION *>(res + offset);
    offset += hpDataset->mNumParticles * sizeof(PRECISION);
...
*/
    ...
    lParList_d.xPos = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.yPos = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.zPos = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.xVel = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.yVel = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.zVel = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.xAcc = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.yAcc = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.zAcc = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
    lParList_d.mass = reinterpret_cast<PRECISION *>(res + offset);
    offset += gpuConfig.localCnt * sizeof(PRECISION);
...
#define ASSIGN(str) \
    str = reinterpret_cast<PRECISION *>(res + offset);      \
    offset += hpDataset->mNumParticles * sizeof(PRECISION)

    res_v = new PRECISION [20 * hpDataset->mNumParticles];
    res = reinterpret_cast<char *>(res_v);
    offset = 0;

    ASSIGN(gParList_h.xPos);
    ASSIGN(gParList_h.yPos);
    ASSIGN(gParList_h.zPos);
    ASSIGN(gParList_h.xVel);
    ASSIGN(gParList_h.yVel);
    ASSIGN(gParList_h.zVel);
    ASSIGN(gParList_h.xAcc);
    ASSIGN(gParList_h.yAcc);
    ASSIGN(gParList_h.zAcc);
    ASSIGN(gParList_h.mass);

    ASSIGN(lParList_h.xPos);
    ASSIGN(lParList_h.yPos);
    ASSIGN(lParList_h.zPos);
    ASSIGN(lParList_h.xVel);
    ASSIGN(lParList_h.yVel);
    ASSIGN(lParList_h.zVel);
    ASSIGN(lParList_h.xAcc);
    ASSIGN(lParList_h.yAcc);
    ASSIGN(lParList_h.zAcc);
    ASSIGN(lParList_h.mass);
...
//    cout << "Launching" << endl;
    ...
    cout << "CONFIG: " << gpuConfig.localCnt << " / " << hpDataset->mNumParticles << endl;
    ComputeParticleAttributes_Kernel <<<num_blocks, num_threads>>> (
...
//    CHK_ERR(    ConvertDeviceToHost());
    ...
    double startd = (double) start.tv_sec + ((double) start.tv_usec / 1000000.0);
    double endd   = (double) end.tv_sec + ((double) end.tv_usec / 1000000.0);
    double ktime  = endd - startd;
    cout << "*** Kernel time: " << ktime << endl;
